"""Behavior tests for agents/task_teacher_v7/main.py.

v5 + bounded Cow/pasture/milk loop. Per
...
BOARD_SIZE = 10
V7_CONFIG = {"episodeSteps": 720, "turnsPerDay": 24}
...
def test_candidate_crops_match_v2():
    module = load_agent_module("task_teacher_v7")
...
def test_emits_buy_land_when_gate_would_pass():
    ...
    action = module.agent(obs, V7_CONFIG)
...
def test_buys_cow_when_empty_pasture_and_cash_after_ne():
    ...
    assert not any(order[:2] == ["BUY_ANIMAL", "GOOSE"] for order in action["market"])
...
def test_never_buys_goose():
    ...
    action = module.agent(obs, V7_CONFIG)
    assert not any(order[0] == "BUY_ANIMAL" and order[1] == "GOOSE" for order in action["market"])
