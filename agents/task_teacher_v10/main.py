"""Kaggriculture multi-tile task/route teacher agent, v10 (v9 + Sheep support + NW quadrant protection + seed-prioritizing cash allocation).
"""
...
def agent(obs, config=None):
    ...
    total_owned_animals = owned_cows + owned_sheep
    total_max_animals = MAX_COWS + MAX_SHEEP
    ...
    animals_in_transit = cow_in_transit + sheep_in_transit
    ...
    wheat_needed_for_feed = stats["unfed_cows"] > 0 or stats["unfed_sheep"] > 0
    ...
    for crop in CANDIDATE_CROPS:
        if crop == "WHEAT" and (owned_cows > 0 or owned_sheep > 0):
            continue
        available = shed.get(crop, 0)
        if available > 0:
            market_orders.append(["SELL", crop, available])
            
    for prod in ["MILK", "WOOL"]:
        if shed.get(prod, 0) > 0:
            market_orders.append(["SELL", prod, shed[prod]])
    ...
    hands_floor = 0
    if me["money"] >= 4000.0 and n_quadrants >= 3:
        hands_floor = 8
    ...
    wants_to_hire = False
    if existing_hands < hands_floor:
        wants_to_hire = True
    else:
        wants_to_hire = should_hire(
            future_load, future_action_turns, me["hires_today"], me["money"], existing_hands=existing_hands, hire_cost_mult=mult
        )

    if wants_to_hire:
        reserved_for_hire = economy.hire_cost(me["hires_today"], mult=mult)
    ...
    liquidity_reserve = 150.0 if (last_day - day >= 2) else 0.0
    essential_reserves = reserved_for_hire + liquidity_reserve
    available_money = max(0.0, me["money"] - essential_reserves)

    # 1. HIRE hands
    if reserved_for_hire > 0.0:
        market_orders.append(["HIRE"])

    # 2. BUY_LAND (up to max 3 extra quadrants)
    land_cost = economy.land_cost(len(me["unlocked_quadrants"]) - 1)
    n_extra = len(me["unlocked_quadrants"]) - 1
    can_buy_land = False
    if land_cost is not None and 0 <= n_extra < 2 and last_day - day >= 12 and day >= 9:
        reserve = 1000.0 if n_extra == 0 else 2000.0
    ...
    if (
        animals_unlocked
        and day + 6 <= last_day
        and owned_sheep < MAX_SHEEP
        and sheep_in_transit < stats["empty_pastures"] + (1 if want_pasture else 0) - cow_in_transit
    ...
    def resolve_unit_action(position: tuple[int, int], task_id, unit_idx: int) -> list:
        ...
        if task_id.kind == TaskKind.PLANT:
            ...
            if seeds_remaining.get(crop, 0) > 0:
                ...
                return ["PLANT", crop]
            if crop not in seed_orders_queued and market_cash_remaining >= economy.CROPS[crop]["seed"]:
                market_orders.append(["BUY_SEED", crop, 1])
                seed_orders_queued.add(crop)
                market_cash_remaining -= economy.CROPS[crop]["seed"]
            return ["PASS"]
