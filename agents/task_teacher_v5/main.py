"""Kaggriculture multi-tile task/route teacher agent, v5 (land-only).

Extends `task_teacher_v2` with ROI-gated NE `BUY_LAND` only, per
docs/superpowers/specs/2026-08-11-task-teacher-v5-design.md.

`MAX_GEESE = 0` disables the Goose path that `task_teacher_v4` added:
ablation (2026-08-10) showed Goose FEED/PICKUP/PLACE labor destroys the
local edge vs v2, while land alone was +0.75 WR in a 10-pair probe.
`task_teacher_v4` is left immutable as the land+Goose experiment.

Same one-time crop scope as v2 (Wheat/Carrot/Melon). No animals, no
fertilizer, no SW/SE. Hire → land → seed budget stack unchanged from v4's
land wiring. Existing `agents/*/main.py` files are never modified.
...
# Land-only: zero geese. Buy/want_coop gates key off this constant, so the
# inherited Goose market/task wiring is inert without further deletes.
MAX_GEESE = 0
