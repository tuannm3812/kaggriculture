...
def agent(obs, config=None):
    ...
    cows_count = owned_cows + cow_in_transit
    feed_reserve = cows_count * 150.0
    hands_reserve = 0.0
    temp_h = 0
    for _ in range(hands_floor):
        hands_reserve += 3.0 * economy.hire_cost(temp_h, mult=mult)
        temp_h += 1
    buffer = 250.0 if cows_count > 0 else 100.0
    minimum_liquidity = feed_reserve + hands_reserve + buffer

    liquidity_reserve = minimum_liquidity if (last_day - day >= 2) else 0.0
    ...
    if hour == 1:
        ...
        for crop in CANDIDATE_CROPS:
            ...
            seeds_needed = max(0, plant_tasks - current_seeds)
            if seeds_needed > 0:
                seed_cost = economy.CROPS[crop]["seed"]
                buy_limit = 25 if day == 0 else 12
                buy_qty = min(seeds_needed, buy_limit, int(market_cash_remaining // seed_cost))
                if buy_qty > 0:
