"""Kaggriculture multi-tile task/route teacher agent, v6 (delayed NE land).

Extends `task_teacher_v5` with a higher land budget reserve so `BUY_LAND`
waits until the bank can absorb $1k without starving Melon cashflow, per
docs/superpowers/specs/2026-08-11-task-teacher-v6-design.md.

`MAX_GEESE = 0` (inherited from v5): land-only, no Goose path.
Same one-time crop scope as v2 (Wheat/Carrot/Melon). No animals, no
fertilizer, no SW/SE. Existing `agents/*/main.py` files are never modified.
